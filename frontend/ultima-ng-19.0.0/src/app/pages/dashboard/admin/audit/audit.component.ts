...
export class AuditComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.loadPointVentes();
    }

    loadPointVentes(): void {
        this.loadingPointVentes = true;
        this.userService
            .getAllPointVentesAudit$()
    ...
    exportFilteredData(table: any, type: string): void {
        ...
        a.href = url;
        a.download = `audit_${type}_${new Date().getTime()}.csv`;
        a.click();
    ...
    exportPDF(): void {
        ...
        try {
            const dateDebut = this.formatDateForAPI(this.auditForm.value.dateDebut);
            const dateFin = this.formatDateForAPI(this.auditForm.value.dateFin);
            const filename = `audit_rapprochement_${dateDebut}_${dateFin}.pdf`;
