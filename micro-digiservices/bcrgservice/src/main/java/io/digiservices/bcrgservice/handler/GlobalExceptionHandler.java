package io.digiservices.bcrgservice.handler;

import feign.FeignException;
import io.digiservices.bcrgservice.domain.ErrorResponse;
...
/**
 * Traduction des erreurs en reponses HTTP coherentes pour la plateforme BCRG.
 *
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleFeign(FeignException ex) {
        ...
        log.warn("[BCRG] appel ebanking en echec (status amont={}) -> {}", ex.status(), status.value());
    ...
    public ResponseEntity<ErrorResponse> handleGeneric(Exception ex) {
        log.error("[BCRG] erreur inattendue : {}", ex.getMessage(), ex);
