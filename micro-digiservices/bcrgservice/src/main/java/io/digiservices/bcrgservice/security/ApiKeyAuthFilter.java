...
/**
 * Authentification par cle API publique pour la plateforme BCRG.
 *
 * <p>Lit le header {@code X-API-Key} et le compare en temps constant a la cle
 * publique configuree ({@code BCRG_PUBLIC_API_KEY}). Ne traite que les URLs
 * {@code /bcrg/**}. Echec -> 401 + log WARN (IP source uniquement).</p>
 */
@Slf4j
public class ApiKeyAuthFilter extends OncePerRequestFilter {

    public static final String API_KEY_HEADER = "X-API-Key";
    private static final String PROTECTED_PREFIX = "/bcrg/";
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (!StringUtils.hasText(providedKey) || !isApiKeyValid(providedKey)) {
            log.warn("[BCRG-PUBLIC-AUTH] Acces refuse a {} - cle API absente ou invalide (source={})",
