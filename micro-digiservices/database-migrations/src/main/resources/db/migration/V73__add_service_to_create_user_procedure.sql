-- V73: Mise à jour de la procédure stockée create_user pour ajouter service='Personnel'

-- Supprimer l'ancienne procédure (avec 11 paramètres)
DROP PROCEDURE IF EXISTS public.create_user(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying);

-- Recréer la procédure avec service='Personnel' par défaut
...
BEGIN
    ...
    -- Insertion de l'utilisateur avec matricule, phone et service='Personnel'
    INSERT INTO users (user_uuid, first_name, last_name, email, username, member_id, matricule, phone, service)
    VALUES (p_user_uuid, p_first_name, p_last_name, p_email, v_generated_username, p_member_id, p_matricule, p_phone, 'Personnel')
...
COMMENT ON PROCEDURE public.create_user(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying)
    IS 'Procédure pour créer un utilisateur avec rôle USER, matricule, phone et service=Personnel';
