-- V107 : Bilan conditionnel selon montant demande
-- Regle metier:
--   montant_demande >= 50 000 000 => Bilan + Flux de tresorerie obligatoires
--   montant_demande <  50 000 000 => Flux de tresorerie uniquement (bilan optionnel)
...
BEGIN
    ...
    -- REGLE METIER : Bilan requis si montant >= 50 000 000
    v_bilan_requis := COALESCE(v_analyse.montant_demande, 0) >= 50000000;
    ...
    -- 4. VALIDATION SECTION 1 - BILAN (CONDITIONNEL selon montant)
    SELECT COUNT(*) INTO v_count
    FROM analyse_bilan
    WHERE analyse_id = p_analyse_id AND type_periode = 'N';

    IF v_count = 0 THEN
        IF v_bilan_requis THEN
            v_erreurs := array_append(v_erreurs,
                'Section Bilan: Bilan periode N absent (obligatoire pour montant >= 50M)');
        ELSE
            v_warnings := array_append(v_warnings,
                'Warning: Bilan periode N absent (optionnel pour montant < 50M)');
        END IF;
    ELSE
        SELECT * INTO v_bilan
        ...
        IF v_bilan.total_actif = 0 THEN
            ...
            ELSE
                v_warnings := array_append(v_warnings,
                    'Warning: Bilan Total actif = 0');
            END IF;
        END IF;
    END IF;

    -- 5. VALIDATION SECTION 2 - RENTABILITE / FLUX DE TRESORERIE (TOUJOURS OBLIGATOIRE)
    SELECT COUNT(*) INTO v_count
...
COMMENT ON FUNCTION fn_soumettre_analyse(BIGINT, VARCHAR, VARCHAR, BOOLEAN, t_personne_caution_input[])
    IS 'V107: Bilan conditionnel - obligatoire si montant >= 50M, optionnel sinon. Flux de tresorerie toujours obligatoire.';
