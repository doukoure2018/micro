...
BEGIN
    -- Vérifier que demandeIndividuel_id existe si fourni
    IF p_demandeIndividuel_id IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM demandeIndividuel WHERE demandeIndividuel_id = p_demandeIndividuel_id) THEN
            RAISE EXCEPTION 'demandeIndividuel_id % n''existe pas', p_demandeIndividuel_id;
END IF;
END IF;
...
END IF;
    ...
    -- 1. Insérer le promoteur (uniquement si des données sont fournies)
    IF p_nom_promoteur IS NOT NULL OR p_prenom_promoteur IS NOT NULL THEN
...
END IF;
    ...
    -- 2. Insérer l'entreprise (uniquement si des données sont fournies)
    IF p_nom_entreprise IS NOT NULL THEN
        ...
        IF v_entreprise_id IS NOT NULL THEN
            ...
            -- 5. Insérer le compte d'exploitation actuel si des données sont fournies
...
END IF;
            ...
            -- 6. Insérer le compte d'exploitation prévisionnel si des données sont fournies
...
END IF;
    ...
    -- 7. Insérer la demande de crédit avec demandeIndividuel_id
    IF v_entreprise_id IS NOT NULL OR p_demandeIndividuel_id IS NOT NULL THEN
        INSERT INTO demande_credit (
            ...
            v_entreprise_id, v_date_courante,
            COALESCE(p_montant_demande, 0), p_duree_mois,
            p_objet_financement, p_delegation_id, p_agence_id, p_point_vente_id,
...
COMMENT ON FUNCTION public.traiter_demande_sans_analyse_bool(
    BIGINT, VARCHAR, VARCHAR, DATE, VARCHAR, VARCHAR, VARCHAR, VARCHAR,
    VARCHAR, VARCHAR, VARCHAR, DATE, VARCHAR, VARCHAR, VARCHAR, VARCHAR,
    NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC,
    DATE, DATE, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC,
    ...
    NUMERIC, INTEGER, VARCHAR, NUMERIC, NUMERIC, VARCHAR, NUMERIC, TEXT,
    BIGINT, BIGINT, BIGINT, BIGINT
) IS 'Fonction modifiée pour traiter une demande de crédit avec liaison demandeIndividuel_id et paramètres optionnels';