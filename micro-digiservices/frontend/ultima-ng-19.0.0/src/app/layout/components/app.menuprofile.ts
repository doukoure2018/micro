import { Component, computed, effect, ElementRef, inject, Input, OnDestroy, Renderer2 } from '@angular/core';
...
@Component({
    ...
    template: `<button (click)="toggleMenu()" pTooltip="Profile" [tooltipDisabled]="isTooltipDisabled()">
            <img [src]="user?.imageUrl" alt="avatar" style="width: 32px; height: 32px;" />
        ...
        <ul *ngIf="menuProfileActive()" [@menu]="isHorizontal() ? 'overlay' : 'inline'">
            ...
            <li (click)="logout()" pTooltip="Logout" [tooltipDisabled]="isTooltipDisabled()">
                <button class="p-link">
